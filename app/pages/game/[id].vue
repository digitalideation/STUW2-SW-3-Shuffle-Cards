...
const readyCount = computed(() => playerList.value.filter(p => p.isReady).length)
const otherPlayers = computed(() => playerList.value.filter(p => p.id !== playerId.value))
...
async function joinGame() {
  ...
  try {
    await callFn('join-game', { roomId, playerId: playerId.value, playerName: playerName.value })
  } catch (e: unknown) {
...
function subscribe() {
  ...
  channel
    .on(
      ...
      (payload) => { game.value = payload.new as GameState }
    )
...
<template>
  <div class="min-h-screen bg-slate-900 text-white flex flex-col">
      ...
      <div class="flex-1 flex items-center justify-center p-4">
        <div class="w-full max-w-sm space-y-6">
          ...
          <div class="text-center">
            <p class="text-slate-500 text-xs uppercase tracking-widest mb-1">Room</p>
            <h1 class="text-4xl font-black font-mono text-white tracking-widest">{{ roomId }}</h1>
            <p class="text-slate-500 text-xs mt-2">Share this code so others can join</p>
          </div>
          ...
          <div class="bg-slate-800 rounded-2xl overflow-hidden">
            ...
            <div v-if="isJoining" class="px-4 py-6 text-center text-slate-500 text-sm">
              Joining…
            </div>
            ...
            <ul v-else class="divide-y divide-slate-700/50">
              ...
              >
                <div class="flex items-center gap-2">
                  ...
                  <span class="text-sm font-medium">
                    {{ p.name }}
                    <span v-if="p.id === playerId.value" class="text-slate-500 text-xs">(you)</span>
                  </span>
                </div>
                <span
              ...
              <li v-if="!playerList.length" class="px-4 py-4 text-slate-500 text-sm text-center">
                Connecting…
              </li>
            </ul>
          </div>
          ...
          >
            <span v-if="isSettingReady" class="inline-block w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin mr-2"></span>
          ...
          <div class="flex items-center justify-center gap-1.5 text-xs text-slate-600">
            <span class="w-1.5 h-1.5 rounded-full" :class="isConnected ? 'bg-emerald-500' : 'bg-yellow-500 animate-pulse'"></span>
    ...
    <template v-else>
      ...
      <!-- Header -->
      <header class="flex items-center justify-between px-4 py-3 bg-emerald-950/80 border-b border-white/10 shrink-0">
        <span class="font-mono font-bold text-emerald-400 tracking-widest text-sm">{{ roomId }}</span>

        <!-- Other players card counts -->
        <div class="flex items-center gap-3">
          <div v-for="p in otherPlayers" :key="p.id" class="flex items-center gap-1.5 text-sm">
        ...
        <div class="flex items-center gap-2 text-sm">
          ...
          <span
            ...
            :class="deckSize === 0 ? 'text-red-400 bg-red-950' : 'text-white bg-white/10'"
          >{{ deckSize }}</span>
        </div>
      </header>

      <!-- Table -->
      <main class="flex-1 bg-emerald-950 flex flex-col items-center justify-center gap-8 p-6 overflow-auto">

        <!-- Other players face-down hands -->
        <div v-if="otherPlayers.length" class="flex gap-8">
          <div v-for="p in otherPlayers" :key="p.id" class="flex flex-col items-center gap-2">
            <span class="text-xs text-white/40 font-medium">{{ p.name }}</span>
            <div class="flex gap-1">
              <div
                v-for="i in p.cardCount" :key="i"
                class="w-12 h-[4.5rem] rounded-lg border border-white/10 shadow"
                style="background: repeating-linear-gradient(135deg,#14532d,#14532d 4px,#166534 4px,#166534 8px)"
              ></div>
              <span v-if="p.cardCount === 0" class="text-white/20 text-xs self-center">no cards</span>
            </div>
          </div>
        </div>
        ...
        <div class="flex flex-col items-center gap-1">
          <div class="relative w-14 h-[4.5rem]">
            <div v-for="i in Math.min(deckSize, 5)" :key="i"
              class="absolute inset-0 rounded-lg border border-white/10"
              style="background: repeating-linear-gradient(135deg,#14532d,#14532d 4px,#166534 4px,#166534 8px)"
              :style="{ transform: `translateY(${-(i-1)*1.5}px)` }"
            ></div>
            <div v-if="deckSize === 0" class="inset-0 absolute flex items-center justify-center">
        ...
        <div class="flex flex-col items-center gap-3">
          <p class="text-xs text-white/30 uppercase tracking-wider">Your hand — tap to discard &amp; draw</p>
          ...
          <div class="flex gap-2 flex-wrap justify-center">
            ...
            >
              ...
              <div class="text-lg font-bold w-full text-center leading-none">{{ card.slice(-1) }}</div>
              ...
              <div v-else
                ...
                <span class="text-[9px] font-bold text-current opacity-50">DISCARD</span>
              </div>
            </button>
          </div>

          <p v-if="myHand.length === 0" class="text-white/20 text-sm">No cards in hand</p>
          <p v-if="deckSize === 0 && myHand.length > 0" class="text-amber-400/70 text-xs">
      ...
      <!-- Footer -->
      <footer class="shrink-0 px-4 py-3 bg-emerald-950/80 border-t border-white/10 flex items-center justify-between">
